...
> **Project Zomboid 線上地塊規劃器** — 專為模組作者、地圖編輯器玩家與生存基地設計師打造的零依賴純前端工具。

## ✨ 特色亮點

- **零依賴純前端**：無需 `npm` / `Vite` 等打包工具或建置步驟，以任何 HTTP 伺服器開啟即可在瀏覽器中直接運行。
- **PZ 遊戲幾何對齊**：
  - 預設 64x64 地圖規模。
  - 輸入遊戲真實絕對座標原點（例如 `10500, 9200`），即時換算地塊對應的 PZ 世界座標。
- **雙視角即時平滑過渡**：
  - **2:1 菱形視角**：北方定為右上角，展現立體空間感。
  - **1:1 正交視角**：北方定為正上方，適合純平面佈局觀察。
